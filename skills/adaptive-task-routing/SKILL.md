...
1. Form a lightweight rough plan sufficient to name the upcoming phase and its likely capability needs. Do not inspect broadly or begin implementation merely to improve the routing estimate.
...
4. For an enabled model gate, load and follow [research-model-router](../research-model-router/SKILL.md) as a coordinator-delegated call, explicitly supplying the resolved effective context and rough plan so its direct-selection guard does not dispatch back. When the generated Gemini dependency appendix is present, use its complete model-router, host, registry, and shared-policy instructions directly. It alone owns model/effort judgment. Load the child even if the host did not independently select it. If neither packaged content, generated appendix, nor a host Skill resource is accessible, report that component as unavailable rather than inventing its result. Never synthesize a child result from the coordinator description or memory.
5. In user-facing output for a gate that precedes ongoing work, write one brief localized sentence stating the understood task and rough approach before the routing note. This preface provides orientation, not a detailed plan, diagnostic trace, or Skill narration. Then present the two router results in one compact routing note. Put the context result first in its own localized conversation-setting block with a plain-language recommendation and an explicit yes/no answer for whether to switch windows. Keep `CURRENT`, `HANDOFF`, and `CLEAN` only in structured evidence; never show those English enum tokens in normal user-facing output. Then show the model router's minimum-sufficient setting, recommended setting, upgrade value, and actual next action. Omit unreadable current model/effort fields. An enabled model gate must not disappear just because both pairs match the current configuration. Explicitly report deferred or unavailable components. Do not label such a gate complete.
6. Continue authorized work or wait only for a decision the selected mode or task scope requires. Put detailed planning, findings, or execution after the routing note. A plan-only request authorizes routing advice and the requested plan, not implementing the plan. Revalidate before execution after a manual switch, changed environment, or materially revised plan.
