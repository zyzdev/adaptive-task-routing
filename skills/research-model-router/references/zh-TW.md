...
先呈現使用者要求的計畫或前置分析，再顯示精簡的任務需求與兩組建議。清單與能力依據足夠但目前設定讀不到時，仍直接給出兩組具體設定，不顯示未知欄位，也不解釋「是否需要切換未知」。只說目前環境能否代為套用，以及符合介面的使用者操作。不可只回傳 `CURRENT / CURRENT`。繁體中文必須逐字使用 `【最低足夠 AI 設定】` 與 `【建議 AI 設定】`，不得改成 Markdown `#` 標題。`ask` 顯示後停止等待自然回覆，不要求固定口令；`auto` 才能套用或沿用設定後在同一回合繼續。
