...
兩組設定後、操作段落前加一則「切換評估：」及任務理由。例如：「切換評估：低。本階段剩餘工作短，預期收益不足以抵銷切換成本，建議維持目前設定。」效益不明時寫：「切換評估：效益尚未確立，暫不自動調整設定。」不顯示讀不到的目前值或診斷來源。保留或延後時操作段落需一致地保留設定；`ask` 等待自然選擇，不又要求套用目標組合。下方手動切換範本適用於值得切換或使用者明確指定目標時。切換評估延後不代表目的地未確定或任務型推薦未完成。

### 選擇操作段落

先依 `switch_assessment.decision` 選擇操作，再套用任何宿主範本。`retain`／`defer` 不附 `/model`、選單步驟或套用目標的邀請；讀不到目前設定本身不能觸發手動切換建議。繁體中文 `ask` 結尾固定使用：「目前保留設定；我先停在這裡，等你決定是否沿用目前設定開始下一階段。」`auto` 保留設定並繼續已授權工作，不提問。其他語言翻譯相同語意。只有 `decision: change` 或使用者明確指定目標才使用下方手動切換範本；此選擇規則也約束備援表格與宿主指引。
...
- `auto`：切換評估支持變更，且模型及強度操作都可呼叫、已授權且可驗證時，自動套用建議；任一操作只能由使用者完成或不可用時，把手動操作列為可選項，沿用目前設定繼續已授權工作。此延續規則只屬於 `auto`；`ask` 一律在建議後停止等待。
...
兩組不同時，升級理由要說明建議組合可增加的品質；兩組相同時，說明為何再提高設定的價值低。只有取得相符即時值或使用者提供值，而且對切換判斷有用時，才顯示目前模型／強度；精簡結果不得顯示 `unknown / unknown`。除非使用者詢問診斷，不提探測、備援／Registry 來源、新鮮度、介面／帳號適用性、無法讀取的目前值、信心、assessment 或 mode，也不以探索機制解釋推薦理由。精簡輸出只保留任務所需的設定區塊、精簡切換評估，以及實際處置。`ask` 不能代為設定時，ChatGPT App 或網頁寫：「目前環境無法代為切換模型與推理強度。如需採用建議，可使用介面中的模型與推理強度選單調整；我先停在這裡，等你決定是否調整，或沿用目前設定開始下一階段。」App 不顯示 CLI 專用的 `/model`。已確認是 Codex CLI 時改用：「目前環境無法代為切換模型與推理強度。如需採用建議，可用 `/model` 調整；我先停在這裡，等你決定是否調整，或沿用目前設定開始下一階段。」Gemini CLI 的 `/model` 只用於模型／別名；Reasoning 使用模型預設時寫：「目前環境無法代為切換模型；Reasoning 使用模型預設。如需採用建議，可用 `/model` 選擇模型；我先停在這裡，等你決定是否調整，或沿用目前設定開始下一階段。」介面未確認時只泛稱介面中的模型控制。這些是自然選擇，不是指定回覆口令。`auto` 無法切換時才把控制列為可選項並沿用目前設定繼續；成功套用且驗證後寫：「已自動套用建議設定，現在繼續執行。」
