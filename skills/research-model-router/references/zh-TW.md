...
顯示精簡的任務需求與兩組建議。清單與能力依據足夠但目前設定讀不到時，仍直接給出兩組具體設定，不顯示未知欄位，也不解釋「是否需要切換未知」。只說目前環境能否代為套用，以及使用者下一步。不可只回傳 `CURRENT / CURRENT`。

繁體中文固定使用下列版型，文字要換成本次實際任務：
