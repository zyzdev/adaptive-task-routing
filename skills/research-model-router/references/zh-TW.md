...
在初步計畫與實際工作 Context 都已確定後、開始大量工作前觸發。交付改善計畫且有具體實質下一階段時，在回覆結束前先給該階段建議，即使實作要等批准；只要求計畫不等於授權實作。直接使用此 Skill 時可沿用目前 Context，不強制先跑另一個 Router。
...
helper 成功時，使用完整模型清單與它能建立的相符即時目前設定。helper 回報 `codex_state_unwritable` 或 `permission_denied` 時，該次嘗試後就停止，不另行要求讀取權限；改讀取 `<plugin-root>/shared/model-catalogs/openai-codex-cli.json`。在 Registry 的 `reference_surfaces` 所列 OpenAI 介面且未超過 `expires_at` 時，直接把官方跨介面能力資料用於建議，不先要求使用者提供選單。清單本身是在 CLI 觀察，因此只證明該次 CLI 的可用性；其他介面的帳號可用性只記為未驗證，不顯示在精簡結果，也不可冒充即時 App 資料。忽略無法確認的目前設定，直接從 Registry 的模型說明與受支援強度產生兩組具體建議。讀取清單不代表能切換；只有 `auto` 且模型／強度操作可呼叫、已授權又可驗證時才套用。否則依已確認介面顯示選單或 CLI 的 `/model`，等待使用者設定完成或決定不調整後回覆「繼續」。保存的 thread 設定與磁碟預設值只是提示，不冒充即時設定。
...
| 已辨識為 OpenAI 介面、Runtime 探測無法完成，但有未過期的內建資料 | 該次讀取後停止，不另行要求權限；直接依官方跨介面能力資料給出兩組具體設定，不先要求使用者抄寫選單。帳號可用性與目前值只在結構化證據標為未驗證。只有 `auto` 且切換操作可呼叫、已授權、可驗證時套用，否則依 App 選單或 CLI `/model` 等實際控制等待使用者調整或決定繼續。 |
...
沒有自動切換工具只影響執行，不妨礙有依據的推薦。目前組合未知而建議具體組合時，`ask` 詢問是否使用該組合，不宣稱是相對於未知現況的升級或降級。
...
- `ask`：未設定模式時使用此預設。已知目前設定符合建議時直接繼續；否則顯示兩組設定及已知操作，等待使用者設定完成或決定不調整並回覆「繼續」。
- `auto`：模型及強度操作都可呼叫、已授權且可驗證時，自動套用建議；任一操作只能由使用者完成或不可用時，提供手動操作並等待「繼續」。

使用者目前回合的明確指定優先。Model 模式與 Context 模式彼此獨立。

## 輸出

顯示精簡的任務需求與兩組建議。清單與能力依據足夠但目前設定讀不到時，仍直接給出兩組具體設定，不顯示未知欄位，也不解釋「是否需要切換未知」。只說目前環境能否代為套用，以及使用者下一步。不可只回傳 `CURRENT / CURRENT`。繁體中文必須逐字使用 `【最低足夠 AI 設定】` 與 `【建議 AI 設定】`，不得改成 Markdown `#` 標題。需要使用者操作時，「繼續」提示必須是回覆最後一段；提示後立即結束，不得再附加計畫、任務分析、分隔線或其他內容。
...
兩組不同時，升級理由要說明建議組合可增加的品質；兩組相同時，說明為何再提高設定的價值低。只有取得相符即時值或使用者提供值，而且對切換判斷有用時，才顯示目前模型／強度；精簡結果不得顯示 `unknown / unknown`。除非使用者詢問診斷，不提探測、備援／Registry 來源、新鮮度、介面／帳號適用性、無法讀取的目前值、信心、assessment 或 mode，也不以探索機制解釋推薦理由。精簡輸出只保留任務所需的設定區塊，以及是否已代為套用或需要使用者操作。需要手動設定時，依實際介面提供操作方式：ChatGPT App 或網頁有模型與推理強度選單時，最後寫：「目前環境無法代為切換模型與推理強度。請使用介面中的模型與推理強度選單完成設定。完成後請回覆『繼續』。若決定不調整，也請回覆『繼續』。」App 不顯示 CLI 專用的 `/model` 指令。已確認是 Codex CLI 時，才以 `/model` 為主要操作。Gemini CLI 的 `/model` 只用於模型／別名；Reasoning 使用模型預設時，最後寫：「目前環境無法代為切換模型；Reasoning 使用模型預設。請用 `/model` 選擇建議的模型；完成後請回覆『繼續』。若決定不調整，也請回覆『繼續』。」介面未確認時只泛稱介面中的模型控制，不提 `/model`。若 `auto` 已成功套用並驗證兩項操作，改寫：「已自動套用建議設定，現在繼續執行。」
