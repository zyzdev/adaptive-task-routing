...
| 已辨識為 OpenAI 介面、Runtime 探測無法完成，但有未過期的內建資料 | 該次讀取後停止，不另行要求權限；直接依官方跨介面能力資料給出兩組具體設定，不先要求使用者抄寫選單。帳號可用性與目前值只在結構化證據標為未驗證。只有 `auto` 且切換操作可呼叫、已授權、可驗證時套用，否則把 App 選單或 CLI `/model` 等實際控制列為可選操作，沿用目前設定繼續已授權工作。 |
...
沒有自動切換工具只影響執行，不妨礙有依據的推薦。目前組合未知而建議具體組合時，`ask` 將操作列為可選項，不宣稱是相對於未知現況的升級或降級，也不因此暫停已授權工作。
...
- `ask`：未設定模式時使用此預設。顯示兩組設定，除非使用者明確要求改變，否則沿用目前設定；需要時把已知操作列為可選項，直接繼續已授權工作。
- `auto`：模型及強度操作都可呼叫、已授權且可驗證時，自動套用建議；任一操作只能由使用者完成或不可用時，把手動操作列為可選項，沿用目前設定繼續已授權工作。

使用者目前回合的明確指定優先。Model 模式與 Context 模式彼此獨立。

## 輸出

顯示精簡的任務需求與兩組建議。清單與能力依據足夠但目前設定讀不到時，仍直接給出兩組具體設定，不顯示未知欄位，也不解釋「是否需要切換未知」。只說目前環境能否代為套用，以及可選的使用者操作。不可只回傳 `CURRENT / CURRENT`。繁體中文必須逐字使用 `【最低足夠 AI 設定】` 與 `【建議 AI 設定】`，不得改成 Markdown `#` 標題。不能代為設定時，不要求使用者回覆特定口令；若已有獲授權的後續工作，沿用目前設定在同一回合繼續。只有模型建議或計畫型請求時，正常結束，不自行擴張任務。
...
兩組不同時，升級理由要說明建議組合可增加的品質；兩組相同時，說明為何再提高設定的價值低。只有取得相符即時值或使用者提供值，而且對切換判斷有用時，才顯示目前模型／強度；精簡結果不得顯示 `unknown / unknown`。除非使用者詢問診斷，不提探測、備援／Registry 來源、新鮮度、介面／帳號適用性、無法讀取的目前值、信心、assessment 或 mode，也不以探索機制解釋推薦理由。精簡輸出只保留任務所需的設定區塊，以及是否已代為套用或可選的使用者操作。不能自動設定時，依實際介面提供操作方式：ChatGPT App 或網頁有模型與推理強度選單時寫：「目前環境無法代為切換模型與推理強度。如需採用建議，可使用介面中的模型與推理強度選單調整；我會先依目前設定繼續執行。」App 不顯示 CLI 專用的 `/model` 指令。已確認是 Codex CLI 時寫：「目前環境無法代為切換模型與推理強度。如需採用建議，可用 `/model` 調整；我會先依目前設定繼續執行。」Gemini CLI 的 `/model` 只用於模型／別名；Reasoning 使用模型預設時寫：「目前環境無法代為切換模型；Reasoning 使用模型預設。如需採用建議，可用 `/model` 選擇模型；我會先依目前設定繼續執行。」只有模型建議或計畫型請求時，把「我會先依目前設定繼續執行」換成「也可沿用目前設定」。介面未確認時只泛稱介面中的模型控制，不提 `/model`。若 `auto` 已成功套用並驗證兩項操作，改寫：「已自動套用建議設定，現在繼續執行。」
