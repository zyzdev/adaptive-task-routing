...
計入剩餘工作、設定、快取重用、延遲、重試、返工及交接成本。快取狀態與傳入的對話內容不同：快取未命中不會刪除歷史，新視窗不是免費切換，只調強度也可能依平台規則影響快取。目前組合未知時暫緩自動切換，仍交付具體任務型建議；快取成本未知不等於零或必然失效，明確品質缺口仍可支持變更。使用者明確指定的目標優先，但不能補足宿主缺少的操作能力。

`auto` 只有合理的 `decision: change` 才能進入已授權、可呼叫且可驗證的套用；retain／defer 保留設定並繼續已授權工作。`ask` 仍在建議後等待，`off` 略過對應 Router。兩組 AI 設定後增加一則在地化切換評估，操作段落需與之相符。共同政策及兩份 Gemini Runtime 投影使用相同規則，不新增服務或持久化活動日誌。
