...
Both routers default to `ask`. A recommendation that retains current state does not interrupt; a recommended change waits for the user's choice unless that router is explicitly set to `auto`.
