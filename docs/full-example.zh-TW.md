...
請在已知的 App 選擇器選擇 high；完成後請回覆「繼續」。若決定不調整，也請回覆「繼續」。目前尚未開始實作。
```

提供建議不等於獲得實作授權。App 選擇器位置未知時，應說明未知，不能捏造精確選單路徑。只有受支援的設定改變被建議時才詢問；留在目前對話時不打斷工作。
...
這不表示已確認目前模型足夠。沒有適用內建參考的宿主，真正需要選擇設定時可以詢問一次實際選擇器選項。已辨識為 OpenAI 介面且有相符、未過期的內建 Registry 時，則直接給出兩組可用性未驗證的具名建議，不先要求使用者抄寫選單，再等待使用者透過 App 選單或 CLI 指令調整，或決定繼續。只有可選清單時，也不能據此猜測目前使用哪個模型。
