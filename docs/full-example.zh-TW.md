...
假設檢查發現初始化過度串行，提出三個相依改動：延後非必要服務、平行讀取獨立資料、加入啟動順序測試。AI 先說明證據與完整改善方案，再辨識出實質**下一階段**：實作並驗證這些改動。對話路由建議留在目前對話；模型路由建議下一階段提高強度。本例也已知剩餘階段短，設定成本高於額外核對的收益，因此切換決定為保留。路由訊息接在計畫之後：
...
切換評估：低。剩餘階段短，收益不足以抵銷設定成本，建議維持目前 medium 強度。
...
如果使用者明確改選 high 並要求開始，該選擇優先於維持建議，不額外要求路由確認。App 選擇器位置未知時，AI 只描述已知操作，不捏造精確選單路徑。
...
- CLI 是 `auto`：切換評估支持變更後，逐項確認設定操作可呼叫、已授權且可驗證才執行；retain／defer 保留設定並繼續已授權工作。互動式指令本身不能證明能力。自動操作失敗一次後提供手動方式。
