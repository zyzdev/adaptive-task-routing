...
讀取能力與切換能力分開判斷。`auto` 只有在模型及強度操作確實可呼叫、已授權且可驗證時才套用建議。否則顯示兩組設定，把符合目前介面的使用者操作列為可選項，沿用目前設定繼續已授權工作，不要求使用者回覆特定口令。ChatGPT App 與網頁只提示可見的模型與推理強度選單，不顯示 CLI 專用的 `/model`；只有已確認的 Codex CLI 才使用該指令。不能只因讀得到清單就宣稱已切換。

## 解析模式與執行者

- `off`：不評估也不輸出該 Router，沿用目前 Context 或模型設定。
- `ask`：執行評估並顯示兩組設定。除非使用者明確要求改變，否則沿用目前設定；需要時把精確的手動操作列為可選項，並直接繼續已授權工作。使用者要求且可驗證的改變，由 AI 執行可呼叫且已授權的操作。
