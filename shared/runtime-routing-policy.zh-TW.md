...
即使設定未知，仍交付下一階段能力與相對推理需求；這不是確認目前模型適合。能確認目的地清單時優先使用，並且始終需要能力證據。在已辨識的 OpenAI 介面，Runtime 探測無法完成時，未過期的內建 Registry 可依官方跨介面能力資料直接產生兩組具名建議；帳號可用性仍只在結構化證據標為未驗證，也不能據此觸發或授權切換。不得先要求使用者抄寫選單才提供建議。已知模型但強度清單未知時，可以只建議模型、強度暫留 `CURRENT` 並說明未知。
...
讀取能力與切換能力分開判斷。`auto` 只有在模型及強度操作確實可呼叫、已授權且可驗證時才套用建議。否則顯示兩組設定與符合目前介面的使用者操作，等待使用者完成後回覆「繼續」；使用者也可決定不調整並直接回覆「繼續」。ChatGPT App 與網頁只提示可見的模型與推理強度選單，不顯示 CLI 專用的 `/model`；只有已確認的 Codex CLI 才使用該指令。不能只因讀得到清單就宣稱已切換。

## 解析模式與執行者

- `off`：不評估也不輸出該 Router，沿用目前 Context 或模型設定。
- `ask`：執行評估。已知目前組合符合建議時直接繼續；否則顯示兩組設定與已知的精確操作，等待使用者設定完成或決定不調整並回覆「繼續」。已驗證的改變獲接受後，可呼叫且可驗證的操作由 AI 執行。
- `auto`：執行評估，並逐項執行已允許、可呼叫且可驗證的操作。只有不支援、無法使用或 `user_only` 的項目降級為明確的使用者動作。因此混合能力可以產生部分自動流程，但每項結果都必須說明實際發生什麼。
