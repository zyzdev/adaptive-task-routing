...
每個宿主在套用模型／強度前都先做此評估。`auto` 只有 `decision: change` 才能由 Router 發起變更，且每項操作仍須已授權、可呼叫、可驗證；`retain` 或 `defer` 保留設定並繼續已授權工作，說明必要的品質限制。使用者明確要求套用特定設定時優先遵循，不額外強制路由確認。`ask` 與 `off` 保留原互動規則。切換評估延後不等於任務型推薦未完成；目的地未確定時才仍依原規則延後 Model Gate。

精簡輸出在兩組 AI 設定後、實際操作段落前，加一則在地化的切換評估。說明維持、值得變更或暫緩自動變更及任務理由；有依據才顯示切換價值，不確定時寫收益尚未確立、暫留設定，不顯示讀不到的目前欄位或診斷來源。建議維持時，操作段落不能又把套用建議組合寫成已值得進行的切換。

操作段落先依切換決定選擇，再使用宿主控制範本。`retain`／`defer` 不附 `/model`、選單步驟或套用目標的邀請；繁體中文 `ask` 結尾寫：「目前保留設定；我先停在這裡，等你決定是否沿用目前設定開始下一階段。」`auto` 則保留並繼續，不提問。只有 `decision: change` 或使用者明確指定目標才使用手動切換範本；讀不到目前設定本身不觸發手動切換建議。此規則同樣約束備援表格與宿主指引。
