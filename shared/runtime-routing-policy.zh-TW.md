...
讀取並遵循 [UX 共用契約](routing-ux.md)。先交付使用者要求的發現或計畫，再於 Routing 區塊依序顯示「建議動作、原因、是否換視窗、必要 AI 設定、實際下一步」。`recommended_setting` 內部名稱不變，外部稱為「任務適配設定」，不代表現在應該切換。

預設 `compact`；要求詳細時使用 `detailed`，不是新的路由模式。精簡版保留啟用的對話／視窗結果及有用的 AI 設定；詳細版增加最低足夠設定、任務適配設定與升級理由。兩組仍在內部計算，不因省略顯示而刪除。`switch_value` 等評分僅供診斷，不新增活動日誌。只有已確認目前設定適合時才可正面顯示「維持目前設定」；資訊不足用「暫時沿用設定」，不能冒稱適合。

`ask` 只在變更環境前或有關鍵阻礙時詢問。保留及不妨礙進度的 defer，直接繼續已授權工作；只要求計畫就完成交付，不開始實作，也不製造保留確認問題。重大品質疑慮、目的地未定或必要需求不足時，問一個能解決問題的具體問題。Context-off 不評估對話或顯示視窗答案；模型保留不能覆蓋尚待決定的交接。使用者已明確選擇的設定不再重複確認，操作仍須有真實能力。
...
- 啟用 Context 時仍明確顯示是否切換視窗，不顯示原始 enum；關閉時省略其評估。
